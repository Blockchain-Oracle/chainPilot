...
/**
 * Input schema for ETH transfer preparation
 */
 ...
 *
 * Validates inputs, converts amounts, estimates gas
 */
async function prepareEthTransfer(
  params: z.infer<typeof schema>
): Promise<PrepareTransactionOutput<EthTransferProps>> {
  const { from, to, amount, chainId, toEnsName } = params;
 ...
 */
export const prepareEthTransferTool = new FunctionTool(prepareEthTransfer, {
  name: 'prepare_eth_transfer',
  description:
    'Prepare a native ETH transfer transaction. Returns transaction details for user approval. Does NOT execute the transaction.',
  parameterTypes: {
    from: 'string',
    to: 'string',
    amount: 'string',
