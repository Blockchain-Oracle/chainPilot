...
/**
 * Input schema for contract call preparation
 */
...
/**
 * Prepare generic contract call transaction
 */
async function prepareContractCall(
  params: z.infer<typeof schema>
): Promise<PrepareTransactionOutput<ContractCallProps>> {
  const { from, contractAddress, functionName, data, value, chainId, contractName, comment } = params;
 ...
 */
export const prepareContractCallTool = new FunctionTool(prepareContractCall, {
  name: 'prepare_contract_call',
  description:
    'Prepare a generic smart contract function call. Requires pre-encoded function data. Returns transaction details for user approval. Does NOT execute the transaction.',
  parameterTypes: {
    from: 'string',
